...
require 'nngraph'
require 'cunn'
require 'utils/OneMinus'

local TRAIN_LOG_WORDS = 100000

local GRU = torch.class('GRU')

function GRU:__init(options)
	self.options = options
end

-- GRU cell activation function
function GRU:GRU(input, prev_h)
	...
	local update_gate = nn.Sigmoid()(input_hidden_sum()) 
	local reset_gate = nn.Sigmoid()(input_hidden_sum())
	local tmp = nn.Linear(self.options.hidden_size, self.options.hidden_size)(nn.CMulTable()({reset_gate, prev_h}))
	local tmp_h = nn.Tanh()(nn.CAddTable()({input, tmp}))
	local hidden = nn.CAddTable()({nn.CMulTable()({nn.OneMinus()(update_gate), prev_h}), nn.CMulTable()({update_gate, tmp_h})})
	return hidden 
end

function GRU:build_net()
	...
	if self.options.layers > 1 then
		...
		for i = 1, self.options.layers do
			...
			local next_hidden = self:GRU(dropped_input, prev_hidden)
			table.insert(next_state, next_hidden)
			net[i] = next_hidden
		end
	else
		local dropped_input = nn.Dropout(self.options.dropout)(net[0])
		local next_hidden = self:GRU(dropped_input, prev_state)
	...
	if self.options.layers > 1 then
		for i = 0, self.options.bptt do
			...
			for j = 1, self.options.layers do
				...
				self.tmp_hist[i][j] = transfer2gpu(torch.zeros(self.options.batch_size, self.options.hidden_size))
			end
		end
		self.grad_h = {}
		for i = 1, self.options.layers do
		...
		self.grad_h = transfer2gpu(torch.zeros(self.options.batch_size, self.options.hidden_size))
		for i = 0, self.options.bptt do
			self.history[i] = transfer2gpu(torch.zeros(self.options.batch_size, self.options.hidden_size)) -- torch tensor is row major, thus batch is set to row
			self.tmp_hist[i] = transfer2gpu(torch.zeros(self.options.batch_size, self.options.hidden_size))
		end
	end
	self.models = make_recurrent(self.core_model, self.options.bptt)
end

function GRU:forward(output_layer)
	local len = 0
	local log_prob = 0
...
function GRU:train_one_epoch(train)
...
function GRU:forward2(output_layer)
	local log_prob = {}
...
function GRU:load_model(input_file)
	self.core_model = torch.load(input_file)
end

function GRU:restore(model)
...
function GRU:save_model(output_file)
