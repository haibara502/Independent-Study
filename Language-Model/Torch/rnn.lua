...
-- RNN cell activation function
function RNN:RNN(input, prev_h)
...
function RNN:build_net()
	...
	if self.options.layers > 1 then
		...
		for i = 1, self.options.layers do
			...
			local next_hidden = self:RNN(dropped_input, prev_hidden)
			table.insert(next_state, next_hidden)
			net[i] = next_hidden
		end
	else
		local dropped_input = nn.Dropout(self.options.dropout)(net[0])
		local next_hidden = self:RNN(dropped_input, prev_state)
...
function RNN:forward(output_layer)
	local len = 0
	local log_prob = 0
...
function RNN:forward2(output_layer)
	local log_prob = {}
